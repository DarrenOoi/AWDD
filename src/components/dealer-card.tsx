...
export function DealerCard({ dealer }: DealerCardProps) {
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-2xl border border-stone-200/80 bg-[var(--bg-elevated)] shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
      <div className="relative aspect-[4/3] bg-gradient-to-br from-stone-100 to-stone-200">
        <Image src={dealer.heroImage} alt={`${dealer.name} hero`} fill className="object-cover" />
        {dealer.featured ? (
